...
public class OAuthTokenStoreService implements TokenStore {
	...
	public OAuth2Authentication readAuthentication(String token) {
		OAuthAccessToken accessToken = mapper.findByTokenId(extractTokenKey(token));
		if (accessToken != null) {
	...
	public void storeAccessToken(OAuth2AccessToken accessToken, OAuth2Authentication authentication) {
		...
		mapper.saveAccessToken(cat);
	}

	@Override
	public OAuth2AccessToken readAccessToken(String tokenValue) {
		OAuthAccessToken accessToken = mapper.findByTokenId(extractTokenKey(tokenValue));
		if (accessToken != null) {
	...
	public void removeAccessToken(OAuth2AccessToken oAuth2AccessToken) {
		OAuthAccessToken accessToken = mapper.findByTokenId(extractTokenKey(oAuth2AccessToken.getValue()));
		if (accessToken != null) {
			mapper.deleteAccessToken(accessToken);
		}
	}
	...
	public OAuth2RefreshToken readRefreshToken(String tokenValue) {
		OAuthRefreshToken refreshToken = mapper.findRefreshTokenByTokenId(extractTokenKey(tokenValue));
		return refreshToken != null ? refreshToken.getTokenObject() : null;
	}

	@Override
	public OAuth2Authentication readAuthenticationForRefreshToken(OAuth2RefreshToken refreshToken) {
		OAuthRefreshToken rtk = mapper.findRefreshTokenByTokenId(extractTokenKey(refreshToken.getValue()));
	...
	public void removeAccessTokenUsingRefreshToken(OAuth2RefreshToken refreshToken) {
		OAuthAccessToken token = mapper.findByRefreshToken(extractTokenKey(refreshToken.getValue()));
		if (token != null) {
			mapper.deleteAccessToken(token);
		}
	}
	...
	public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
		...
		OAuthAccessToken token = mapper.findByAuthenticationId(authenticationId);
	...
	public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
		...
		List<OAuthAccessToken> result = mapper.findByClientIdAndUsername(clientId, userName);
	...
	public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
		Collection<OAuth2AccessToken> tokens = new ArrayList<OAuth2AccessToken>();
		List<OAuthAccessToken> result = mapper.findByClientId(clientId);
		result.forEach(e -> tokens.add(e.getTokenObject()));
		return tokens;
	}
